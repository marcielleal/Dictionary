	...
	for(int i=0;i<mi_Length;i++){
		...
		if(aux>_x){
			if (max==_x) max=aux;
				...
				max=aux;
		}else if(aux==_x) flag=true;
	}
	if(max==_x||!flag) return false;	//Se nao achou maior||nao existe no dicionario
	_y=max;
	return true;
}
template <typename Key,typename Data,typename KeyComparator>
bool DictionaryAL<Key,Data,KeyComparator>::predecessor(Key _x, Key &_y) const{
	...
	if(min==_x||!flag) return false;	//Se nao achou menor||nao existe no dicionario
	_y=min;
	return true;
}
